##########################################################################################################################
#    1.  Program Title  : BUMclrGBMMdelDvlp.py
# GBM
...
for idxRecCode in [
    ...
    S3_CLIENT.upload_file(Filename = f"{modelDirectory}/mdl_Rec{idxRecCode}Rat3.pkl"
                        , Bucket   = bucket
                        , Key      = f"{s3ChampionModelDir}/mdl_Rec{idxRecCode}Rat3.pkl")    

    S3_CLIENT.upload_file(Filename = f"{modelDirectory}/mdl_Rec{idxRecCode}Rat3_{baseYm_bfr_1m}.pkl"
                        , Bucket   = bucket
                        , Key      = f"{s3modelDirectory}/mdl_Rec{idxRecCode}Rat3_{baseYm_bfr_1m}.pkl")
                        
    S3_CLIENT.upload_file(Filename = f"{modelDirectory}/compare_eval.csv"
                        , Bucket   = bucket
                        , Key      = f"{s3modelDirectory}/모델평가지표_모니터링_{baseYm_bfr_1m}.csv")
